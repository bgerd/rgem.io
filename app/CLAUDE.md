...
### `src/types/grid.ts`
- Type definitions: `ConnectionStatus`, `RgbColor`, `GridState`
...
- `selectedRgemIdRef` synced from URL-derived `gemId` in the routing effect — `App.tsx`
...
### Routing (`App.tsx`)
URL is the source of truth for which gem is active. `useRoute` reads `window.location.pathname` and re-renders on `popstate`. `parseGemIdFromPathname` extracts the gemId segment.
```
/              gemId=null  → modal visible, no connection
/{gemId}       gemId≠null  → modal hidden, auto-connect triggered
/{invalid}                 → silent replaceState to /
```
On connection failure: `history.replaceState({ gemId, error: "connection_failed" }, "", "/")` — modal mounts and reads this state to pre-populate the input with a contextual error.

### ConnectionStatus (`App.tsx`)
```
"idle" ──routing effect (gemId≠null)──> "connecting" ──success──> "connected"
  ^                                          │
  │                                          └──failure──> (redirect to /) "idle"
  └──routing effect (gemId=null)─────────────────────────────────────────────────
```
...
```bash
# NOTE: app/.env must exist before running the dev server — it is generated by
# ./configure.sh <env> (run from the repo root). Without it, the app starts but
# every connection attempt times out because VITE_WS_URL is undefined.
