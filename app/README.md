...
- **Node.js 18+** and npm
- Run `./configure.sh <env>` from the **project root** first — this generates `app/.env` with the correct WebSocket and API URLs for your target environment. Do not create `.env` manually.
...
Generated by `./configure.sh <env>` into `app/.env` (gitignored):
