...
- **Event listeners inside `.then()`**: In `App.tsx:301`, visibility/online/offline listeners are registered inside `ensureConnected("app_mount").then(...)`. This is intentional — it prevents race conditions during initial mount. Do not move them outside the `.then()`.
